"""
无人机智能化应用系统 - 分组作业6 最终版
功能：地图显示、障碍区圈选记忆、航线规划、安全半径绕行、实时飞行监控、飞行轨迹绘制
"""
...
def init_session_state():
    ...
    # 新增：已飞轨迹点
    if 'flown_path' not in st.session_state:
        st.session_state.flown_path = []
init_session_state()
...
def create_safe_bypass(start_lat_lng, end_lat_lng, obstacle_points_lat_lng, safety_radius_m, direction):
    ...
    bypass_point = point_at_distance(center_lat, center_lng, perp_bearing, safety_radius_m + 10)
...
def reset_flight_task():
    ...
    st.session_state.flight_battery = 100
    st.session_state.flight_drone_pos = None
    st.session_state.flown_path = []  # 重置已飞轨迹
...
def start_flight_task():
    ...
    st.session_state.flown_path.append(st.session_state.planned_route[0])
...
def simulate_flight_step():
    ...
    if len(route) > 1:
        ...
        if idx >= len(route) - 1:
            ...
            st.session_state.flown_path.append((lat, lng))
    return True
...
def create_map():
    ...
    # 绘制已飞轨迹（绿色）
    if st.session_state.flown_path:
        flown_wgs = []
        for point in st.session_state.flown_path:
            wgs = gcj02_to_wgs84(point[0], point[1])
            flown_wgs.append([wgs[0], wgs[1]])
        folium.PolyLine(
            locations=flown_wgs, color='green', weight=4, opacity=1.0,
            popup="已飞轨迹"
        ).add_to(m)
...
def main():
    ...
    st.caption("魏坤的《无人机智能化应用2451》 | 分组作业6-项目Demo")
    loaded,count,timestamp = False,0,None
    ...
    # 三栏布局：地图 | 控制面板 | 航线分析（和参考图一致）
    map_col, ctrl_col, analysis_col = st.columns([3,1,1])
    ...
    with map_col:
        ...
        st.caption("📌 红色区域为障碍物 | 黄色区域为安全缓冲区 | 蓝色线为规划航线 | 绿色线为已飞轨迹 | 橙色飞机为无人机实时位置")
